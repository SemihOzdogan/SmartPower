...
export const GetMeters = async (x, start, length, refreshPage) => {
    if (refreshPage == 1) {
        start = 0
    }
    ...
    var URL = url + '/meters' + '?start=' + start + '&length=' + length + '&filter[measuring_location_name]=' + location + '&filter[comm_device_id]=' + device_id + '&filter[measuring_device_id]=' + mesauringDevice
    console.log(URL)
    ...
    return fetch(URL, {
        ...
        .then((responseJson) => {
            ...
            x.setState({
                SayacSayisi: responseJson.Data.records_total,
                dataLoading: false
            });

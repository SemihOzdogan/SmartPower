...
class AnalizorListScreen extends Component {
    ...
    UNSAFE_componentWillMount() {
        GetAnalyzers(this, 0, this.state.stopHere, 0);
    }
    ...
    loadMore = () => {
        if ((this.state.dataSource.length > 4) && (this.state.AnalizorSayisi != this.state.dataSource.length))
            if (!this.state.loading) {
                ...
                GetAnalyzers(this, varStartHere, varStopHere, 0);
            }
    }

    loadMoreSpinner = () => {
        if (this.state.dataSource.length != this.state.AnalizorSayisi) {
            return (
            ...
            return (
                <View style={{ flex: 1, paddingBottom: 20 }}>
                    <Text style={{ width: "100%", padding: 20, textAlign: "center", marginTop: 20, fontFamily: 'Poppins-Light', color: '#999' }}>Daha fazla analizörünüz bulunmamaktadır</Text>
                </View>
            )
        }
    }
    ...
    _renderItem = ({ item }) => (
        <CardList
            ...
            routeTitle="Ayarlar"
            routeIcon="cog"
            onPressDeviceAuthOrSettings={() => this.props.navigation.navigate('AnalizorSettings', {
                analizorNo: item.measuring_device_id,
...
export default AnalizorListScreen;

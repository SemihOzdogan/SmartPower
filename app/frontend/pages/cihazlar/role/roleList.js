...
class RoleListScreen extends Component {
    ...
    UNSAFE_componentWillMount() {
        GetRelays(this, 0, this.state.stopHere, 0);
    }
    ...
    loadMore = () => {
        if ((this.state.dataSource.length > 4) && (this.state.RoleSayisi != this.state.dataSource.length))
            if (!this.state.loading) {
                ...
                GetRelays(this, varStartHere, varStopHere, 0);
            }
    }

    loadMoreSpinner = () => {
        if (this.state.dataSource.length != this.state.RoleSayisi) {
            return (
            ...
            return (
                <View style={{ flex: 1, paddingBottom: 20 }}>
                    <Text style={{ width: "100%", padding: 20, textAlign: "center", marginTop: 20, fontFamily: 'Poppins-Light', color: '#999' }}>Daha fazla röleniz bulunmamaktadır</Text>
                </View>
            )
        }
    }
    ...
    _renderItem = ({ item }) => (
        <CardList
            ...
            routeTitle="Ayarlar"
            routeIcon="cog"
            onPressDeviceAuthOrSettings={() => this.props.navigation.navigate('RelaySettings', {
                relayNo: item.measuring_device_id,
            ...
            <Container>
                ...
                <View style={{ flex: 1, backgroundColor: "#f5f5f5" }}>
                    ...
                    {!this.state.dataLoading &&
                        <View style={{ flex: 1 }}>
                            <RoleToolBar
                                ListCount={this.state.RoleSayisi}

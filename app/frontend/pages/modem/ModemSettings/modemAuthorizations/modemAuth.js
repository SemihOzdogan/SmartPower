...
import { View, TouchableOpacity, SafeAreaView, FlatList, Text, TextInput, RefreshControl } from 'react-native';
import { Container, Root, Spinner, Icon, } from 'native-base';
import { GetComdeviceAuth, PostComdeviceAuth, DeleteComdeviceAuth } from '../../../../../backend/modemController/GetModemAuth';
import LinearGradient from 'react-native-linear-gradient';
import Modal from 'react-native-modal';
import * as Animatable from 'react-native-animatable';

class ModemAuthScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            dataSourceAuth: [],
                ...
                <LinearGradient start={{ x: 1, y: 1 }} end={{ x: 0, y: 0 }} colors={['#ADE5FE', '#FFFFFF']} style={{ flex: 1, backgroundColor: '#f5f5f5', padding: 5, justifyContent: 'flex-start', alignItems: 'flex-start', marginTop: 5, borderRadius: 4 }}>
                    <View style={{ borderLeftWidth: 5, borderColor: '#80d8ff', flexDirection: 'row', marginTop: 5 }}>
                        ...
                        <View style={{ width: 50, height: 50, backgroundColor: 'white', justifyContent: 'center', alignItems: 'center' }}>
                            <TouchableOpacity onPress={() => { DeleteComdeviceAuth(item.user_id, this.props.modemNo); this.refreshDataAuth() }}>
                ...
                <Container>
                    ...
                    <View style={{ flex: 1 }} >
                        <View style={{ flex: 1, backgroundColor: "#f5f5f5" }}>
                            ...
                            {!this.state.dataLoading &&
                                <SafeAreaView showsVerticalScrollIndicator={false} style={{ flex: 1, backgroundColor: '#f5f5f5', padding: 10, paddingVertical: 10, paddingHorizontal: 10 }}>
                                    {this.state.dataSourceAuth == "" &&
                                        < View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', }}>
                                            <Text style={{ fontSize: 16,padding:10, borderRadius: 6, backgroundColor: '#FCF8E3', fontFamily: 'Poppins-Light', textAlign: 'center', color: '#8a6d3b' }}>Yetkili kullanıcı bulunmamaktadır</Text>
                                        </View>
                                    }
                                    {this.state.dataSourceAuth != "" &&
                                        <FlatList
                                            ...
                                            renderItem={this._renderItemDataAuth}
...
export default ModemAuthScreen;

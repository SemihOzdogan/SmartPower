...
import { GetNoConnectionDevices } from '../../../backend/baglantiHatalariController/GetNoConnectionDevices';
import LinearGradient from 'react-native-linear-gradient';
import CheckInternet from '../../../backend/InternetController/CheckInternet';

class CihazList extends Component {
    constructor(props) {
        super(props);
        this.state = {
            dataSource: [],
    ...
    UNSAFE_componentWillMount() {
        ...
        GetNoConnectionDevices(this, 0, this.state.stopHere, 0);
    }
    ...
    loadMore = () => {
        if ((this.state.dataSource.length > 4) && (this.state.bk_CihazSayisi != this.state.dataSource.length))
            if (!this.state.loading) {
                ...
                GetNoConnectionDevices(this, varStartHere, varStopHere, 0);
            }
    }

    loadMoreSpinner = () => {
        if (this.state.dataSource.length != this.state.bk_CihazSayisi) {
            return (
            ...
            return (
                <View style={{ flex: 1, paddingBottom: 20 }}>
                    <Text style={{ width: "100%", padding: 20, textAlign: "center", marginTop: 20, fontFamily: 'Poppins-Light', color: '#999' }}>Daha fazla hatalı cihazınız bulunmamaktadır</Text>
                </View>
            )
        }
                    ...
                    <Col>
                        ...
                        <Row style={{ marginTop: 10, borderWidth: 0.5, borderColor: '#ddd', backgroundColor: '#eee', borderTopRightRadius: 6, borderTopLeftRadius: 6 }}>
                            <Col style={{ alignItems: 'center' }}>
                                <Text style={{ fontSize: 12, fontFamily: 'Poppins-Light', textAlign: "center" }}>Cihaz No  <Text style={{ fontSize: 10, color: '#999' }}>{item.measuring_device_id}</Text></Text>
                            </Col>
                            <Col style={{ alignItems: 'center' }}>
                        ...
                        <Row style={{ marginTop: 5, borderWidth: 0.5, borderColor: '#ddd', borderBottomRightRadius: 6, borderBottomLeftRadius: 6 }}>
                            ...
                            <Col style={{ alignItems: 'center' }}>
                                <Icon type="FontAwesome5" name="clock" style={{ color: '#DA7C62', fontSize: 18 }}><Text style={{ fontSize: 12, fontFamily: 'Poppins-Light' }}> Son Bağlantı <Text style={{ fontSize: 14, color: "#999", fontFamily: 'Poppins-Light' }}>{item.measuring_last_packet_time}</Text> </Text></Icon>
                            </Col>
            ...
            <View style={{ flex: 1, backgroundColor: "#f5f5f5" }}>
                ...
                {!this.state.dataLoading &&
                    <View style={{ flex: 1, backgroundColor: '#eee' }}>
                        <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 0, y: 2 }} colors={['white', '#BDBDD7']} style={{ width: '100%', height: 50, flexDirection: 'row' }}>
                            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'flex-start', paddingLeft: 25 }}>
                                <Badge style={{ backgroundColor: 'gray', justifyContent: 'center', alignItems: 'center' }}>
                                    <Text style={{ color: 'white' }}>{this.state.bk_CihazSayisi}</Text>
                                </Badge>
                            </View>
                        ...
                        <Text style={{ padding: 10, textAlign: 'center', fontSize: 16, fontFamily: 'Poppins-Light' }}>Aşağıda listelenmiş <Text style={{ color: 'red', fontFamily: 'Poppins-Light' }}> {this.state.bk_CihazSayisi} adet </Text><Text style={{ fontWeight: '700' }}>cihaz </Text >modemle bağlantı kuramıyor</Text>

                        <SafeAreaView showsVerticalScrollIndicator={false} style={{ backgroundColor: "#eee", flex: 1 }}>
